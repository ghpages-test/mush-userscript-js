...
MESH.initLang = function () {
    ...
    try {
        var translationRequest = new Request("https://api.github.com/repos/Javiernh/MESH/contents/translate/locales/es/translation.json");
        fetch(translationRequest).then(response => {
            response.text().then(text => {
                var parsed = JSON.parse(text);
                var translationData = window.atob(parsed.content);
